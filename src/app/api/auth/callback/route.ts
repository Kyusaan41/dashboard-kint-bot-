...
export async function GET(request: Request) {
  ...
  // Fetch user's guilds to get the guildId
  let guildId = null;
  try {
    ...
    if (guildsRes.ok) {
      ...
      // Find the guild with the ID from the URL or use the first one if none specified
      const requestedGuildId = searchParams.get('guildId');
      if (requestedGuildId) {
        const guild = guilds.find((g: any) => g.id === requestedGuildId);
        if (guild) {
          guildId = guild.id;
        }
      } else if (guilds.length > 0) {
        // Use the first guild if no specific guild was requested
        guildId = guilds[0].id;
      }
    }
  } catch (e) {
    console.error('[auth/callback] error fetching guilds', String(e));
  }
  ...
  (async () => {
    try {
      ...
      if (!guildId) {
        console.warn('[auth/callback] no guildId found, skipping role grant');
        return;
      }

      // Create payload for bot with uid and guildId
      const payload = {
        uid: userJson.id,
        guildId: guildId,
      ...
      const payloadForBot = { token: payloadB64, sig: hmac };
      console.log('[auth/callback] calling bot role endpoint', { roleEndpoint, guildId });
      const botRes = await fetch(roleEndpoint, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payloadForBot),
