...
export async function GET() {
    try {
        // On lance les deux appels en parall̬le pour gagner du temps
        const [leaderboardRes, serverInfoRes] = await Promise.all([
            fetch(`${BOT_API_URL}/points`), // R̩cup̬re le classement { userId, points }
            fetch(`${BOT_API_URL}/serverinfo`) // R̩cup̬re la liste des membres { id, username, avatar }
        ]);
