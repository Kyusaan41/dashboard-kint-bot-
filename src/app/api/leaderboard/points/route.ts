...
export async function GET() {
    try {
        const response = await fetch(`${BOT_API_URL}/points`);

        if (!response.ok) {
            const errorText = await response.text();
            console.error(`Erreur de l'API du bot [${response.status}]: ${errorText}`);
            return NextResponse.json({ message: "Erreur lors de la récupération du classement des points depuis le bot." }, { status: response.status });
        }
